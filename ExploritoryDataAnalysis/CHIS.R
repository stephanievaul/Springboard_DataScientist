...
## Data Cleaning ##
  ...
  ## Multiple Histograms ##  
    ...
    # The color scale used in the plot (provided)
    BMI_fill <- scale_fill_brewer("BMI Category", palette = "Reds")
    
    # Theme to fix category display in faceted plot (provided)
    fix_strips <- theme(strip.text.y = element_text(angle = 0, hjust = 0, vjust = 0.1, size = 14),
                        strip.background = element_blank(), 
                        legend.position = "none")
    
    # Histogram, add BMI_fill and customizations (provided through fix_strips)
    ggplot(adult, aes (x = SRAGE_P, fill= factor(RBMI))) + 
      geom_histogram(binwidth = 1) +
      fix_strips + ## Added from this + down
      BMI_fill +
      facet_grid(RBMI ~ .) +
      theme_classic()
...
## Alternatives ##
    ...
    # Plot 1 - Count histogram (provided)
    ggplot(adult, aes (x = SRAGE_P, fill= factor(RBMI))) + 
      geom_histogram(binwidth = 1) +
      BMI_fill
    ...
    # Plot 4 - Faceted density histogram, from Plot2
    ggplot(adult, aes (x = SRAGE_P, fill= factor(RBMI))) + 
      geom_histogram(aes(y=..density..),binwidth = 1) +
      BMI_fill +
      facet_grid(RBMI ~ .)
    
    # Plot 5 - Density histogram with position = "fill", from Plot2
    ggplot(adult, aes (x = SRAGE_P, fill= factor(RBMI))) + 
      geom_histogram(aes(y=..density..),binwidth = 1,position="fill") +
      BMI_fill
    
    # Plot 6 - The accurate histogram, from Plot5
    ggplot(adult, aes (x = SRAGE_P, fill= factor(RBMI))) + 
...
## Merimeko/Mosaic Plot ##
    ...
    # Copy row names (Ages) to variable X
    DF$X <- row.names(DF)
...
## Adding statistics ##  
    ...
    DF_all <- merge(DF_melted, resid)   ## , by = c("FILL","X")
